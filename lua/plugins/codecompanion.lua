        ...
        interactions = {
            ...
            chat = {
                ...
                keymaps = {
                    ...
                    -- cycle_acp_mode: 循环切换 ACP 权限模式
                    -- 包含 bypassPermissions，以便切换到完全放行模式
                    -- 具体逻辑见 lua/plugins/codecompanion/acp_mode.lua (与 lualine 指示器共用)
                    -- 注意: 只绑定普通模式。插入模式的 <S-Tab> 被 Neovim 默认用于 snippet 向后跳转，
                    --       覆盖它会破坏 vim.snippet.jump(-1)
                    cycle_acp_mode = {
                        modes = { n = "<S-Tab>" }, -- 普通模式下按 Shift+Tab 循环切换
        ...
        adapters = {
            ...
            http = {
                ...
                deepseek = function()
                    ...
                    -- 注意: url 是适配器的顶层字段, 不能写在 env 里
                    -- env 中只有被 ${xxx} 引用的键才生效
                    -- 适配器默认 url = https://api.deepseek.com/chat/completions
